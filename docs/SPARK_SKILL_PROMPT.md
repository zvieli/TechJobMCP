...
The eleven MCP source badges are `[HireMeTech]`, `[Comeet]`, `[Greenhouse]`,
`[Lever]`, `[Workday]`, `[Eightfold]`, `[DirectTech]`, `[LinkedIn]`,
`[Jobify]`, `[GotFriends]`, and `[AllJobs]`.

## Live tool contract

The server exposes exactly 16 tools. Scheduled runs use the one-call tool; the
...
2. `get_job_matches(sources, tech_stack, work_mode, location, min_salary, keywords, exclude_keywords, cv_path, force_refresh, limit, detail_level)` retrieves and scores multi-source jobs via System 1 neural triage.
...
4. `list_job_sources()` returns diagnostic metadata and bounded source health for all 11 registered sources. Do not put it on the scheduled run's critical path.
...
7. `auto_apply_job(job_id, cv_path)` inspects and stages an application preview using the Hybrid Application Dispatcher (Easy Apply, Comeet iFrame, Headless Playwright, or Direct API POST) with zero-cost regex heuristic form mapping and System 2 tailored assets.
8. `confirm_auto_apply(job_id, cv_path, force)` executes submission through the optimal strategy only after every identity, location (Israel/Remote), match score (>=85), fail-closed (`AUTO_APPLY_ENABLED=false`), and SQLite `ApplicationLedger` duplicate gate passes. Captures 4-layer verification receipts (DOM, URL, HTTP, Screenshot).
9. `get_application_history(limit, status)` inspects the persistent SQLite `ApplicationLedger` audit history and receipt proofs.
10. `mark_job_as_applied(job_id, source, company, title, notes)` records external applications in the audit ledger.
11. `calibrate_selectors(force_recalibrate)` diagnoses HireMeTech UI selectors.
12. `search_linkedin_jobs(keywords, location, work_mode, limit, ...)` searches the LinkedIn source directly.
13. `get_linkedin_job_details(job_id)` returns full LinkedIn details and a direct application URL.
14. `notify_new_jobs(channel, ...)` sends unseen jobs through a configured notifier.
15. `test_notifier(channel)` checks notifier health and delivery.
16. `run_job_scout(action_mode, sources, tech_stack, work_mode, location, min_salary, keywords, exclude_keywords, cv_path, force_refresh, limit, detail_level, top_tier_threshold, strong_match_threshold, disqualify_threshold, max_applications)` performs the complete scheduled workflow (scout across 11 sources, System 1 neural scoring, System 2 tailoring, bookmark, and hybrid auto-apply with 4-layer receipts) in one stateless call.
...
1. Call `run_job_scout` exactly once through `@TechJobMCP` with:
   ...
   - `max_applications=3`
   *(Do NOT pass `sources` parameter so that all 11 registered sources are dynamically queried).*
...
- Details for submitted applications (confirmation text, screenshot proof path, and HTTP status)
- System 2 interview preparation questions and personalized talking points for top-tier opportunities
